...
public class EConceptUtility
{
	...
	{
		EConcept eConcept = createConcept(conceptPrimordialUuid, time, status);
	...
	private UUID setupWbPropertyMetadata(String refsetSynonymName, String refsetValueParentSynonynmName, PropertyType pt, DataOutputStream dos) throws Exception
	{
		...
		{
			UUID refsetSynonymNameUUID = Type5UuidFactory.get(refsetSynonymName + " (foundation metadata concept)");
		...
		createAndStoreMetaDataConcept(pt.getPropertyTypeReferenceSetUUID(), pt.getPropertyTypeReferenceSetName(), false, specialSCTMetadataUuidCache.get(refsetSynonymName), dos);
		...
		{
			UUID refsetValueParentSynonymNameUUID = Type5UuidFactory.get(refsetValueParentSynonynmName + " (foundation metadata concept)");
		...
		return createAndStoreMetaDataConcept(ConverterUUID.createNamespaceUUIDFromString(pt.getPropertyTypeReferenceSetName() + "s", true), 
				pt.getPropertyTypeReferenceSetName() + "s", false, specialSCTMetadataUuidCache.get(refsetValueParentSynonynmName), dos).getPrimordialUuid();
	}
}
